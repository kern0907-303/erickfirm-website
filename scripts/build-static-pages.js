...
const serviceMap = { i8: 'enterprise-doctor', nas: 'life-number', abl: 'personal-growth', erick: 'erick-column' };
...
const encodedPath = (service, slug) => `/insights/${service}/${encodeURIComponent(slug)}`;
...
for (const article of articles) {
  const service = article.service || serviceMap[article.brand_id] || 'personal-growth';
  const slug = article.slug || slugify(article.title) || article.id;
  const pagePath = encodedPath(service, slug);
  const filePath = `/insights/${service}/${slug}`;
  ...
  const html = `<!doctype html><html lang="zh-Hant"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>${esc(title)}</title><meta name="description" content="${esc(description)}"><link rel="canonical" href="${canonical}"><meta property="og:title" content="${esc(title)}"><meta property="og:description" content="${esc(description)}"><meta property="og:url" content="${canonical}"><meta property="og:image" content="${domain}/og-default.png"><meta property="og:type" content="article"><meta property="og:site_name" content="Erick Firm"><meta property="og:locale" content="zh_TW"><meta name="twitter:card" content="summary_large_image"><meta name="twitter:title" content="${esc(title)}"><meta name="twitter:description" content="${esc(description)}"><meta name="twitter:image" content="${domain}/og-default.png">${article.aeo_schema || ''}</head><body><div id="root"><main><article><h1>${esc(article.title)}</h1><p>${esc(description)}</p></article></main></div>${assetScript}</body></html>`;
